...
usage() {
    ...
    echo "  -r, --ref     Masked/TE-augmented scanning reference REF/DVS.masked.34TE.fasta. Usually no need to modify."
    echo "  -u, --unmask  Unmasked reference REF/DVS.masked.34TE.fasta used for split-read discovery and depth. Usually no need to modify."
    echo "  -1, --read1   First read file (FASTQ)"
    echo "  -2, --read2   Second read file (FASTQ)"
    echo "  -o, --out     Output prefix"
    echo ""
    echo "Optional options:"
    echo "  -b, --bed     BED file with known ILs default as ${DIR}/known_ILs_bed/SWO_ILs_merged.bed. Usually no need to modify."
...
if [[ (! -f "$STEP1_DONE") || (! -f "${OUT_PREFIX}.split_reads.txt") || (! -f "$STEP1_BAM") ]]; then
    ...
    samtools view -@ "$THREADS" -F 0x900 "$STEP1_UNMASK_BAM" | awk '/SA:Z:/ {print $1}' | sort -u > "${OUT_PREFIX}.split_reads.txt"
