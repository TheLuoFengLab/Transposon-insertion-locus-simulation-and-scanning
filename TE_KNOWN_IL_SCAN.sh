...
> ${SAMP}_${NAME}.bed
# Line 20: Uses 'samtools view' to extract reads from the BAM file within the specified interval (IL)
...
# Line 21: Begins an awk command, passing in variables x and y set to the values of LEFT and RIGHT.
...
# Line 22: Checks if the CIGAR string in the supplementary alignment indicates soft clipping (S) followed by matching (M),
...
# Line 23: Matches a pattern where matching (M) is followed by soft clipping (S) in the CIGAR string.
  # Captures the length of the matching segment.
  # Calculates and prints the appropriate genomic coordinates and read name.
# Line 24: Matches a CIGAR string pattern with soft clipping, matching, and then soft clipping.
  # Captures the lengths of these segments.
# Line 25: Determines which clipping is larger and prints coordinates and read name accordingly.
  # Line 26: The output from awk is redirected to a BED file named based on the sample identifier (SAMP) and the provided name tag (NAME).
